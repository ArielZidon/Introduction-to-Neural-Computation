...
class Som:
    ...
    def fit_square(self, data):
        for i in range(self.n_iterations + 1):
            ...
            if i % 100 == 0:  # plot the map every 100 iterations
                if self.network_dimensions[1] == 1:
    ...
    def plot_net(self, t):
        ...
        if self.network_dimensions[1] == 1:
            for i in range(self.network_dimensions[0]):
                ax.plot(neurons_x[i], neurons_y[i], 'b-', markersize=0, linewidth=1)
        else:
            for i in range(self.network_dimensions[0]):
                xh = []
                yh = []
    ...
    def fit_donut(self, data):
        for i in range(self.n_iterations + 1):
            ...
            l_rate = self.learning_rate * np.exp(-i / self.n_iterations)  # learning rate
            # update weight vector to move closer to input
            # and move its neighbors in 2-D vector space closer
            for x in range(self.net.shape[0]):
                for y in range(self.net.shape[1]):
                    w = self.net[x, y]
                    x_dist = np.abs(x - bmu_idx[0])
                    y_dist = np.abs(y - bmu_idx[1])
                    x_dist = min(x_dist, self.net.shape[0] - x_dist)  # handle edge cases
                    y_dist = min(y_dist, self.net.shape[1] - y_dist)  # handle edge cases
                    w_dist = np.sqrt(x_dist ** 2 + y_dist ** 2)
                    if w_dist <= r:
                        # calculate the degree of influence (based on the 2-D distance)
                        influence = np.exp(-w_dist / (2 * (r ** 2)))
                        # new w = old w + (learning rate * influence * delta)
